...
class MemberRepositoryTest {
    ...
    /*
        ...
        수정 매서드
        ex) id가 2 인 멤버의 이름을 BC 로 바꾼다 가정했을 경우

        UPDATE member
        SET name = 'BC'
        WHERE id = 2;

        JPA 에서 데이터를 수정하는 방식은 추후에 SQL 에서 배우게 될 트랜잭션 내에서
        데이터를 수정해야한다.
                -> 메서드만 사용하는 것이 아닌 @Transactional 애너테이션을
                    메서드 추가해야한다.`
    ...
    @Test
    void update(){
        //given id 2인 객체 가지고 와서 member 라는 객체명에 저장
    ...
    /*
        이상의 메서드에는 @Transactional 애너테이션 없음
            -> @DataJpaTest 애너테이션 때문
            -- 이미 상속되어있음
     */
